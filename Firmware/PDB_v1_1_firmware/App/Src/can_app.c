...
#include "myusart.h"
#include "hsd_drive.h"
#include "voltage_monitor.h" //为了获取电压返回原始值ADC1_Data[]，发给仪表
// app
#include "can_app.h"
// freertos
...
void CAN_Data_Send_App(void)
{
	...
	for (;;) // 出问题原因：多个事件set\clear复用同一个uxBits
	{
		...
		if ((flag1 > 100) | (flag2 > 100) | (flag3 > 100))
		{
			...
			if (CAN_Transmit((const void *)AV_Data, 32, 0x0F146C72) == 1)
			{
...
void CAN_Recv_App(void)
{
	...
	uint8_t CAN_TX_core_state_data[4];
		...
		if ((uxBits & BIT_0) != 0)
		{
			...
			for (i = 0; i < 8; i++)
			{
				if (xQueueReceive(CanReceiveQueueHandle,
								  &Command_Buffer[i],
								  portTICK_PERIOD_MS) == pdFAIL) // portTICK_PERIOD_MS:实时 portMAX_DELAY：无限延时
					myprintf("can_app.c:queue receive error\r\n");
			}
			// 由HSD控制任务处理
			uxBits = xEventGroupSetBits(					  // 跳转至其他地方执行控制
				CanReceiveEventHandle,						  // The event group being updated.
				(const EventBits_t)(Command_Buffer[0] << 8)); // The bits being set.

			// FAN1控制（现在实际上是FAN3控制 豪哥接错线了）[跟随FAN1]//该风扇驱动与DRS供用定时器
			if(Command_Buffer[1]>=50)
				HAL_GPIO_WritePin(GPIOB, GPIO_PIN_6, 1);		//FAN1_PWR_EN（改为FAN3输出）
			else 
				HAL_GPIO_WritePin(GPIOB, GPIO_PIN_6, 0);		//FAN1_PWR_EN（改为FAN3输出）

			if(Command_Buffer[2]==100)
				Command_Buffer[2]=99;
			// FAN2控制 PWM TIM9 CH1 配置为20Hz
			__HAL_TIM_SET_COMPARE(&htim9, TIM_CHANNEL_1, Command_Buffer[2] * 2 ); // 配置比较寄存器(相对于200的自动重装载值)
			HAL_TIM_PWM_Start(&htim9, TIM_CHANNEL_1);								 // 开启PWM输出

			if(Command_Buffer[1]==100)
				Command_Buffer[1]=99;
			// FAN3控制（现在实际上是FAN1控制）TIM9 CH2 配置为20Hz
			__HAL_TIM_SET_COMPARE(&htim9, TIM_CHANNEL_2, Command_Buffer[1] * 2 ); // 配置比较寄存器(相对于200的自动重装载值)
			HAL_TIM_PWM_Start(&htim9, TIM_CHANNEL_2);								 // 开启PWM输出

			if(Command_Buffer[4]==100)
				Command_Buffer[4]=99;
			// 尾灯控制 改为L1_24V备份输出（为TIM1 CH4） 配置为100Hz
			__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_4, Command_Buffer[4] ); // 配置比较寄存器(相对于100的自动重装载值)
			HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_4);							 // 开启PWM输出
			
//			Command_Buffer[5]=100;
//			if(Command_Buffer[5]==100)
//				Command_Buffer[5]=99;
//			// 水泵控制 为TIM2 CH2 配置为20Hz
//			__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_2, Command_Buffer[5] * 2 ); // 配置比较寄存器(相对于100的自动重装载值)
//			HAL_TIM_PWM_Start(&htim2, TIM_CHANNEL_2);

			// 设备状态数据转发仪表(可能的隐患：要求周期性控制指令（来于VCU\仪表），否则仪表上低压电池电压数据不能实时更新)
			CAN_TX_core_state_data[0] = 0; // 初始化
			if (Command_Buffer[5] != 0)
				CAN_TX_core_state_data[0] = CAN_TX_core_state_data[0] | BIT_2; // 水泵开启
			if ((Command_Buffer[0] & 0b110000) == 1)
				CAN_TX_core_state_data[0] = CAN_TX_core_state_data[0] | BIT_1; // DRS开启
			if (Command_Buffer[1] != 0)
				CAN_TX_core_state_data[0] = CAN_TX_core_state_data[0] | BIT_3; // PUMP风扇开启
			if (Command_Buffer[2] != 0)
				CAN_TX_core_state_data[0] = CAN_TX_core_state_data[0] | BIT_4; // 电池箱风扇开启

			CAN_TX_core_state_data[1] = (ADC1_Data[0] * 8 / 1000); // ADC1_Data[0]存储的通道IN14返回值（两电池实际电压值（发送的话高位在前 低位在后））

			// CAN发送一包数据,且发送成功(发数据包ID：0x0F146A72)
			if (CAN_Transmit((const void *)CAN_TX_core_state_data, 4, 0x0F146A72) == 1)
			{
				myprintf("can_app.c:vbat : %d\r\n",&ADC1_Data[0]);
			}
		}
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
	...
	if (hcan == &hcan1)
	{
		...
		{
			// 接收PDB控制指令报文（一包报文8字节内，首字节为设备供电开关信号，后两字节为风扇PWM控制信号）
			if (RxHeader.ExtId == 0x00000202)
			{
				for (i = 0; i < 8; i++)
				{
					...
					xReturn = xQueueSendFromISR(CanReceiveQueueHandle,
												&RxData[i], // 存放指向ADC2_Data地址的指针(问题：可一次把数组里多组数据发出？)
												0);
				}
			...
			else if (RxHeader.ExtId == 0x1234FFFF)
			{
				for (i = 0; i < 1; i++)
				{
					xResult = xEventGroupSetBitsFromISR(
						CanReceiveEventHandle, /* The event group being updated. */
						(RxData[i] << 16),	   /* The bits being set. */
						&xHigherPriorityTaskWoken);
				}
			}
		}

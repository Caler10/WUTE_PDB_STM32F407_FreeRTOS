...
void HSD1_Monitor_App(void)
{
	...
	extern osThreadId_t OLEDmonitorTaskHandle;
	UBaseType_t x = 0;
	BaseType_t xReturn = pdPASS; /* 定义一个创建信息返回值，默认为pdPASS */
	EventBits_t uxBits;			 // 读取事件组的返回值（已配置事件组宽度为24bit）

//	vTaskDelete( CPUTaskHandle );
	vTaskDelete( OLEDmonitorTaskHandle );
...
void BAT_Monitor_App(void)
{
	...
	for (;;)
	{
		...
		{
			// 得实际电压值*10
			// 计算：ADC电压值：V=x/4096*3.3	通道输出电压值*10=x*0.008056640625,取*0.008（偏差：0.0232001536V）
